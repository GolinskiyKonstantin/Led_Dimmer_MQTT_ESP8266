...
const char* const ssidAP PROGMEM = "ESP_Led";     // Имя сети в режиме точка доступа
...
boolean firstStartLed = true;       // Флаг для того чтобы при старте модуля первый раз сразу считать данные с топиков и неждать время переподключения
...
const uint8_t maxStrParamLength = 32;    // Максимальная длина задаваемых параметров настроек
          ...
          const char* const topicLedArg PROGMEM = "topicLed";
          const char* const topicPWMArg PROGMEM = "topicPWM";
          ...
          const char* const statusLed PROGMEM = "status";
          const char* const wifiRssi PROGMEM = "wifirssi";

String ssid, password, domain;
String mqttServer, mqttUser, mqttPassword, mqttClient = "ESP_Led", mqttTopicLed = "/Led", mqttTopicPWM = "/PWM"; // Указываем параметры по умолчанию
String nameEspLed = "ESP Led", aHostname = "ESP8266-Led";  // Указываем параметры по умолчанию
...
uint32_t timeEncoderMqtt = 0;
boolean encoderTimeMqttFlag = false;
...
boolean readConfig() {
  ...
  offset = readEEPROMString(offset, nameEspLed);
  ...
  offset = readEEPROMString(offset, mqttTopicLed);
  offset = readEEPROMString(offset, mqttTopicPWM);
  EEPROM.get(offset, LedOnBoot);
...
void writeConfig() {
  ...
  offset = writeEEPROMString(offset, nameEspLed);
  ...
  offset = writeEEPROMString(offset, mqttTopicLed);
  offset = writeEEPROMString(offset, mqttTopicPWM);
  EEPROM.put(offset, LedOnBoot);
  EEPROM.commit();
}
//***********************************************************************************
...
<body>\n\
  ...
  message += F("\">?</span><br/>\</p>\n\
    ...
    <input type=\"button\" value=\"Led Setup\" onclick=\"location.href='/Led';\" />\n\
...
<body>\n\
  ...
  message += String(maxStrParamLength);
  message += F(" value=\"");
  message += quoteEscape(nameEspLed);
  message += F("\" />\n\ 
    <br/>\n\
...
<body>\n\
  ...
  message += F("\" />\n\
    <br/>\n\
    Topic Led (format MQTT => /Topic Led):<br/>\n\
    <input type=\"text\" name=\"");
  message += FPSTR(topicLedArg);
  message += F("\" maxlength=");
  message += String(maxStrParamLength);
  message += F(" value=\"");
  message += quoteEscape(mqttTopicLed);
  message += F("\" />\n\
      <br/>\n\  
      Topic PWM (format MQTT => /Topic PWM):<br/>\n\
    <input type=\"text\" name=\"");
  message += FPSTR(topicPWMArg);
  message += F("\" maxlength=");
  message += String(maxStrParamLength);
  message += F(" value=\"");
  message += quoteEscape(mqttTopicPWM);
  message += F("\" />\n\
      <br/>\n\                         
    <p>\n\
    <input type=\"submit\" value=\"Save\" />\n\
...
<body>\n\
  <form name=\"Led\" method=\"get\" action=\"/store\">\n\
    <h3>Led Setup</h3>\n\
    State on boot:<br/>\n\
...
void handleStoreConfig() {
  ...
  for (uint8_t i = 0; i < httpServer.args(); i++) {
    ...
    if (argName.equals(FPSTR(ssidArg))) {
      ...
      mqttClient = argValue;
    } else if (argName.equals(FPSTR(topicLedArg))) {
      mqttTopicLed = argValue;
    } else if (argName.equals(FPSTR(topicPWMArg))) {
      mqttTopicPWM = argValue;
    } else if (argName.equals(FPSTR(onbootArg))) {
      LedOnBoot = argValue.toInt();
    }
  }
  #ifdef TestSerialPrint
  Serial.println(F(")"));
...
void mqttCallback(char* topic, uint8_t* payload, uint32_t length) {
  ...
  if (! strncmp(topicBody, mqttTopicLed.c_str(), mqttTopicLed.length())) {
    switch ((char)payload[0]) {
      ...
      case 'T':
        if( onOff ){
         ...
         mqtt_publish(pubsubClient, mqttTopicLed, String(onOff), true); 
         break;  
               
      default:
        boolean Led = false;
        mqtt_publish(pubsubClient, String(topic), String(Led), true);
    }
  }
...
bool mqttReconnect() {
  ...
  if ( (millis() > lastTime + timeoutMQTT) || firstStartLed ) {
    firstStartLed = false;
    #ifdef TestSerialPrint
    ...
    if (result) {
      ...
      // Resubscribe
              ...
              if( flagNoConnectMQTT && flagPressButton ){
                  ...
                  boolean Led = onOff;
                    String topicLed;
                    topicLed += mqttTopicLed;
                  mqtt_publish(pubsubClient, topicLed, String(Led), true);
      ...
      String topicLed;
      topicLed += mqttTopicLed;
      bool resultLed = mqtt_subscribe(pubsubClient, topicLed);

      String topicPWM;
      topicPWM += mqttTopicPWM;
      bool resultPWM = mqtt_subscribe(pubsubClient, topicPWM);

      result=(resultLed and resultPWM);
...
void buttonPush() {
  ...
  onOff=!onOff;
   onOff ? analogWrite(LedPin, LEDbridge) : analogWrite(LedPin, 0);
    if (mqttServer.length() && pubsubClient.connected()) {
        String topicLed;
        topicLed += mqttTopicLed;
      mqtt_publish(pubsubClient, topicLed, String(onOff), true);
...
void setup() {
  ...
  httpServer.on("/Led", handleLedConfig);
...
void loop() {
  ...
  if(onOff){
          ...
          if (newPosition != oldPosition && newPosition != 0) {
            ...
            analogWrite(LedPin, LEDbridge);
            encoderTimeMqttFlag = true;
            timeEncoderMqtt = millis();
          }
  }
  else myEnc.write(0); // Данным методом сбрасываем значения на 0

  if(encoderTimeMqttFlag and ((millis()-timeEncoderMqtt)>1000) ){
    encoderMqtt();
    encoderTimeMqttFlag=false;
  }
delay(5);   
//////////////////////////////////////////////////////////////////////////////////////////////////
}
